...
std::shared_ptr<quesync::message> quesync::client::modules::messages::send_message(
    std::string content, std::string channel_id) {
    packets::send_message_packet send_message_packet(content, channel_id);
...
std::vector<quesync::message> quesync::client::modules::messages::get_channel_messages(std::string channel_id, unsigned int amount, unsigned int offset)
{
    ...
    // Parse the messages
    std::vector<quesync::message> messages = response_packet->json().get<std::vector<quesync::message>>();
