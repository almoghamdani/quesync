...
Executer::Executer(std::function<nlohmann::json()> func, const Napi::Promise::Deferred& deferred) :
    Napi::AsyncWorker(get_fake_callback(deferred.Env()).Value()), // Init the AsyncWorker with a fake callback
    _func(func),
    _deferred(deferred)
{
}
