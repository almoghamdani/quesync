...
class StartPage extends Component {
	...
	connectBtnClicked(event) {
		...
		electron.ipcRenderer.on("client-connect-callback", (event, error) => {
			...
			if (error) {
				...
				} else if (error == window.errors.NO_CONNECTION) {
					// If the client has no internet connection
					this.setState({
						connectError: "You don't have an active internet connection!"
					});
				} else {
					this.setState({
						connectError:
	...
	loginBtnClicked(event) {
		...
		if (this.refs.loginForm[0].value.length == 0) {
			...
			return;
		} else if (this.refs.loginForm[1].value.length == 0) {
		...
		var timeline = anime.timeline({
			...
			complete: () => {
				...
				window.client
					.login(this.refs.loginForm[0].value, this.refs.loginForm[1].value)
					.then(({ user }) => {
						console.log(user);
	...
	newAccountBtnClicked() {
		...
		// Enable all events to the register form
		this.refs.registerForm.style.pointerEvents = "";
	...
	render() {
		return (
			<ThemeProvider
				...
				<ParticlesNoUpdate
					className="quesync-bg-particles"
					params={{
						particles: {
							number: {
								value: 80,
				...
				<Elevation
					...
					style={{
						pointerEvents:
							this.state.connecting || this.state.loggingIn ? "none" : ""
					}}>
					...
					<div
						...
						<form
							className="quesync-form quesync-connect-form"
							ref="connectForm"
							onSubmit={this.connectBtnClicked}>
							<Typography
								use="headline2"
								style={{
							...
							<TextField
								invalid={this.state.serverIPError}
								outlined
								label="Server IP"
							...
							<Button
								...
								{this.state.connecting ? (
									<ButtonIcon icon={<CircularProgress theme="secondary" />} />
								) : null}
								Connect
							</Button>
							<div className="quesync-error-holder">
								<Typography
									className="quesync-connect-error"
									use="caption"
									style={{
										color: "#ff1744",
										paddingTop: "40px",
						...
						<form
							className="quesync-form quesync-register-form"
							ref="registerForm"
